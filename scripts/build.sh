...
stop_build() {
    ...
    echo "Check $arcus_directory/build_log"
    exit -1
}
...
build_and_install() {
  ...
  if [ -z "$source_dir/$module_dir" ] || [ ! -d "$source_dir/$module_dir" ]; then
    echo "invalid module dir : $source_dir/$module_dir" >> $arcus_directory/build_log
    exit 1
  fi

  pushd $source_dir/$module_dir >> $arcus_directory/build_log

  echo "configure options : $configure_options" >> $arcus_directory/build_log
  ./configure --prefix="$target_dir" $configure_options 1>> $arcus_directory/build_log 2>&1
  printf "$module_dir make clean start"
  make clean 1>> $arcus_directory/build_log 2>&1
  printf "\r$module_dir make clean succeed\n"
  printf "$module_dir make start"
  make 1>> $arcus_directory/build_log 2>&1
  printf "\r$module_dir make succeed\n"
  printf "$module_dir make install start"
  make install 1>> $arcus_directory/build_log 2>&1
  printf "\r$module_dir make install succeed\n"

  popd >> $arcus_directory/build_log
}
...
build_all() {
  ...
  if [ -z "$target_dir" ] || [ ! -d "$target_dir" ]; then
    echo "invalid target dir : $target_dir" >> $arcus_directory/build_log
    exit 1
  fi

  if [ -z "$source_dir" ] || [ ! -d "$source_dir" ]; then
    echo "invalid source dir : $source_dir" >> $arcus_directory/build_log
    exit 1
  fi
  ...
  easy_install -a -d $pythonpath kazoo 1>> $arcus_directory/build_log 2>&1
  printf "\rpython kazoo library install succeed\n"
  printf "python jinja2 library install start"
  easy_install -a -d $pythonpath jinja2 1>> $arcus_directory/build_log 2>&1
  printf "\rpython jinja2 library install succeed\n"
  # FIXME pycrypto-2.6 is really really slow.. So let's downgrade it.
  ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future easy_install -a -d $pythonpath pycrypto==2.4.1 1>> $arcus_directory/build_log 2>&1
  printf "python fabric library install start"
  easy_install -a -d $pythonpath fabric==1.8.3 1>> $arcus_directory/build_log 2>&1
  printf "\rpython fabric library install succeed\n"
  pushd $target_dir/scripts >> $arcus_directory/build_log
  if [ ! -f fab ]; then
    ln -s ../lib/python/site-packages/fab fab 1>> $arcus_directory/build_log 2>&1
  fi
  popd >> $arcus_directory/build_log
}
...
if [ -f "$arcus_directory/build_log" ]; then
  rm $arcus_directory/build_log
fi
...
if [ -z "$TARGET_DIR" ]; then
  ...
  if [ ! -d "$TARGET_DIR" ]; then
    mkdir -p $TARGET_DIR
    echo "created a working directory $TARGET_DIR." >> $arcus_directory/build_log
  fi
fi

echo "working directory is $TARGET_DIR." >> $arcus_directory/build_log
